...
export const GroupKickModal = () => {
  const [leaveStep, setLeaveStep] = useState<"accept" | "kick">("accept");
  const isOpen = useAppSelector(modalSelectors.isOpen);
  const type = useAppSelector(modalSelectors.selectType);

  const { toggleModal } = useActions();

  const isModalOpen = isOpen && type === EModalVariables.GROUP_KICK_MODAL;
      ...
      <DialogContent className="border rounded-3xl py-6 md:py-8 overflow-hidden">
        <div className="flex flex-col items-center space-y-1 mb-2">
          ...
          <div className="text-center text-[13px] w-full">
            {leaveStep === "accept" &&
              `Ты хочешь удалить Магомедов Мариф из группы БИВТ-24-12`}
            {leaveStep === "kick" && (
              <div className="w-full space-y-2">
                <p>Чтобы удалить человека из группы, введи его имя</p>
                <FloatingLabelInput
                  label="Имя"
